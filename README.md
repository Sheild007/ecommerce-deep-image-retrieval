...
1. **Training Phase**: Train model on product images using metric learning losses
2. **Embedding Generation**: Pre-compute embeddings for all 44,000 catalog images
3. **Indexing**: Store embeddings with scikit-learn NearestNeighbors (cosine similarity)
4. **Query**: User uploads image → compute embedding → find K-nearest neighbors in embedding space
5. **Results**: Return top-10 most similar products with similarity scores
